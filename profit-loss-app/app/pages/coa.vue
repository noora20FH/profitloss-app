...
const categoryOptions = computed(() => {
    return Array.isArray(categories.value) ? categories.value.map(cat => ({
        value: cat.id,
        label: cat.name
    })) : [];
});
...
<template>
  <div class="p-8 max-w-5xl mx-auto my-10">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-green-700">Master Chart of Accounts (COA)</h1>
      <button 
    ...
    <!-- Loading State -->
    <div v-if="coaPending || catPending" class="p-4 text-center text-gray-500">Memuat data...</div>
    <!-- Error State -->
    <div v-else-if="coaError || catError" class="p-4 bg-red-100 text-red-700 rounded-lg">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="item in coas" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-semibold">{{ item.code }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ item.name }}</td>
            <!-- Tampilkan nama Kategori dari relasi -->
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <span :class="{'bg-blue-100 text-blue-800': item.category}" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
    ...
    <!-- MODAL (CREATE & EDIT) -->
    <div v-if="showModal" class="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 transition-opacity">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform transition-all duration-300 scale-100">
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <!-- Input Kategori ID -->
          <div class="mb-4">
            <label for="category_id" class="block text-sm font-medium text-gray-700">Kategori COA</label>
            <select
                id="category_id"
            ...
            <p v-if="validationErrors.category_id" class="mt-1 text-sm text-red-600">{{ validationErrors.category_id[0] }}</p>
          </div>
          
          <!-- Input Kode -->
          <div class="mb-4">
            <label for="code" class="block text-sm font-medium text-gray-700">Kode Akun (e.g., 401)</label>
            <input 
              id="code"
              v-model="form.code" 
              type="text" 
              ...
              required
            >
            <p v-if="validationErrors.code" class="mt-1 text-sm text-red-600">{{ validationErrors.code[0] }}</p>
          </div>

          <!-- Input Nama -->
          <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-700">Nama Akun (e.g., Gaji Karyawan)</label>
            <input 
              id="name"
              v-model="form.name" 
              type="text" 
          ...
          <div class="flex justify-end space-x-3">
            ...
            <button 
              ...
              :class="isSubmitting ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'"
            >
